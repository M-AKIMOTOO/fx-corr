...
以下は修正前の診断記録。現在の本体は `--vlbi` 指定時に、この公称対流圏モデルを読み出し整列と毎秒の遅延テーブルへ加える。テーブルの微分・四次補間を通して、整数／小数サンプル遅延と搬送波位相を同じモデルで補正する。`--vlbi` を指定しない場合の動作は従来どおり。

Rust の大気遅延値と Python の独立計算を360時刻で比較し、差が1e-16秒未満であることを検査する。修正後は元の入力を再相関し、frinZ の rate/acel を再推定する必要がある。以前の係数は大気未補正の出力に対する値なので、そのまま再利用しない。実データの再相関はこの環境では未実施であり、実測気象を含まない公称モデルで残差が完全にゼロになるとは保証しない。
